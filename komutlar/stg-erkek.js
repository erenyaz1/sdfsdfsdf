...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = message.guild.roles.cache.find(
    r => r.id === "812996545682735133"
  );
  const erkekrol2 = message.guild.roles.cache.find(
    r => r.id === "812996545682735132"
  );
  const kayıtsız = message.guild.roles.cache.find(
    r => r.id === "812996545682735128"
  );
  const genelchat = message.guild.channels.cache.find(
    c => c.id === "812996547397812256"
  );
  const savelog = message.guild.channels.cache.find(
    c => c.id === "812996549524324378"
  );
  ...
  if (!erkekrol2)
    return message.channel.send(
      new MessageEmbed()
        ...
        .setDescription(
          `2. Erkek rolü ayarlanmamış/yanlış id girilmiş kayıt işlemine devam edilemiyor.`
        )
        .setColor("#a22a2a")
    );
...
Sunucumuzun **\n ``${tag}``** tagını alarak ailemizin parçası olabilirsin.`
  );
