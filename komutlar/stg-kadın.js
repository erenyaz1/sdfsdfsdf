...
exports.run = async (client, message, args) => {
  ...
  const kadınrol = message.guild.roles.cache.find(
    r => r.id === "812996545690599437"
  );
  const kadınrol2 = message.guild.roles.cache.find(
    r => r.id === "812996545690599436"
  );
    ...
    return message.channel
      .send(
        new MessageEmbed()
          ...
          .setDescription(
            `2. Kadın rolü ayarlanmamış/yanlış id girilmiş kayıt işlemine devam edilemiyor.`
          )
          .setColor("#a22a2a")
      )
      .then(x => x.delete({ timeout: 5000 }));
